...
use base64::Engine;
use chrono::Duration;
type DartFnFuture<T> = std::pin::Pin<Box<dyn std::future::Future<Output = T> + Send + 'static>>;
...
impl RequestClient {
    ...
    pub fn new(settings: ClientSettings) -> Result<RequestClient, RhttpError> {
    ...
    pub async fn client_for_url(&self, url: &Url) -> Result<reqwest::Client, RhttpError> {
