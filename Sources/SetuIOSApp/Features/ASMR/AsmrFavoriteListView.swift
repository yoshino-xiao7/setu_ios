...
struct AsmrFavoriteListView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: SetuSpacing.lg) {
                ...
                if pager.phase == .loadingInitial {
                    ...
                    SetuSectionHeader(title: "我的 ASMR 收藏", subtitle: "共 \(pager.total) 部")
                    LazyVGrid(columns: columns, spacing: SetuSpacing.md) {
                        ForEach(pager.items) { item in
                            Button { router.navigate(to: .asmrWork(item.externalId)) } label: {
                                VStack(alignment: .leading, spacing: 8) {
                                    SetuImageTile(urlString: item.coverUrl, accessibilityLabel: item.title, aspectRatio: 1)
                                    Text(item.title)
        ...
        .navigationTitle("ASMR 收藏")
        .task { await loadFirstPage() }
        .refreshable { await loadFirstPage(clearExisting: true) }
        .accessibilityIdentifier("asmr.favorites.page")
    }
    ...
    private func loadFirstPage(clearExisting: Bool = false) async {
        await pager.loadFirstPage(clearExisting: clearExisting) { page in
            let result = try await environment.moduleFavoriteClient.list(module: .asmr, page: page, size: pager.pageSize)
